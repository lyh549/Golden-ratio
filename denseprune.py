...
y_mask = y.gt(0)
zero_num = y_mask.shape[0] - torch.sum(y_mask)
print('The zero_num is {}'.format(zero_num))
print('The zero_num percent is {}'.format(zero_num / total))
if zero_num > thre_index:
    print('Can cut more channel, the percent is {}'.format(zero_num / total))
...
for k, m in enumerate(model.modules()):
    if isinstance(m, nn.BatchNorm2d):
        ...
        if torch.sum(mask) == 0:
            cfg.append(int(1))
            index0 = torch.argmax(weight_copy)
            mask[index0] = 1
